...
#
  ...
  instance reelaySensor1: ReelayComponents.ReelaySensor base id 0x0E200 \
    {
      phase Fpp.ToCpp.Phases.configComponents """
        using namespace ReelayComponents;
        reelaySensor1.set_data_channel_name(0, "v_I", ReelayDataType::Number);
        reelaySensor1.set_data_channel_name(1, "v_Delta", ReelayDataType::Number);
        reelaySensor1.set_data_channel_name(2, "burnrate", ReelayComponents::ReelayDataType::Number);
        reelaySensor1.set_data_channel_name(3, "stage", ReelayComponents::ReelayDataType::String);
        reelaySensor1.set_data_channel_name(4, "height", ReelayComponents::ReelayDataType::String);
        """
   }

  instance RocketSensor1: Ref.RocketSensor base id 0x0E400

  instance rocketSimulator: Ref.RocketSimulator base id 0xE600 #\
    #queue size Default.queueSize \
    #stack size Default.stackSize \
    #priority 95

  instance reelayLogger: Svc.ActiveLogger base id 0x0E800 \
    queue size Default.queueSize \
    stack size Default.stackSize \
    priority 98

  instance reelayTlm: Svc.TlmChan base id 0x0F00 \
    queue size Default.queueSize \
    stack size Default.stackSize \
    priority 97
}
